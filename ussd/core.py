...
class UssdRequest(object):
        ...
        **kwargs,
        ...
        if use_built_in_session_management and session_id is not None:
            raise InvalidAttribute(
                "When using built_in_session_management "
                "has been enabled session_id should "
                "be None"
            )
        if use_built_in_session_management and expiry is None:
            raise InvalidAttribute(
                "When built_in_session_management has been" "enabled expiry should not be None"
            )
    ...
    def get_session_from_store(self) -> SessionStore:
        return SessionStore(session_key=self.session_id, kv_store=self.session_store_backend)
    ...
    def get_screens(self, screen_name=None):
        return self.journey_store.get(self.journey_name, self.journey_version, screen_name)
...
class UssdHandlerMetaClass(type):
    def __init__(cls, name, bases, attr, **kwargs):
        super(UssdHandlerMetaClass, cls).__init__(name, bases, attr)
        ...
        if not abstract or attr.get("screen_type", "") == "custom_screen":
            ...
            for attribute in required_attributes:
                if attribute not in attr and not hasattr(cls, attribute):
                    raise MissingAttribute("{0} is required in class {1}".format(attribute, name))

            if not isinstance(attr["serializer"], SchemaMeta):
                raise InvalidAttribute(
                    "serializer should be a "
                    "instance of {serializer}".format(serializer=SchemaMeta)
                )
...
class UssdHandlerAbstract(object, metaclass=UssdHandlerMetaClass):
    ...
    def __init__(
        ...
        self.pagination_more_option = self._add_end_line(
            self.get_text(self.pagination_config.get("more_option", "more\n"))
        )
        self.pagination_back_option = self._add_end_line(
            self.get_text(self.pagination_config.get("back_option", "back\n"))
        )
        self.ussd_text_limit = self.pagination_config.get(
            "ussd_text_limit", ussd_airflow_variables.ussd_text_limit
        )
    ...
    def route_options(self, route_options=None):
        ...
        if self.screen_content.get("with_items"):
            loop_items = (
                self.evaluate_jija_expression(
                    self.screen_content["with_items"], session=self.ussd_request.session
                )
                or loop_items
            )
        ...
        for item in loop_items:
            ...
            if isinstance(loop_items, dict):
                extra_context.update(
                    dict(key=item, value=loop_items[item], item={item: loop_items[item]})
                )

            for option in route_options:
                if isinstance(option, dict) and self.evaluate_jija_expression(
        ...
        return self.ussd_request.forward(self.screen_content["default_next_screen"])
    ...
    def get_context(cls, session, extra_context=None):
        ...
        context.update(dict(now=datetime.datetime.now(), datetime=datetime))

        # add all built in functions
        context.update(_built_in_functions)
        return context

    @staticmethod
    def render_text(session, text, context=None, extra=None, encode=None):
        if context is None:
            context = UssdHandlerAbstract.get_context(session)
    ...
    def get_text(self, text_context=None):
        ...
        if isinstance(text_context, dict):
            language = (
                ...
                if self.ussd_request.language in text_context.keys()
        ...
        if self.raw_text:
            return text_context
        return self.render_text(self.ussd_request.session, text_context)

    @classmethod
    def evaluate_jija_expression(
        cls,
        expression,
        session,
        extra_context=None,
        ...
        if not isinstance(expression, str) or (
            lazy_evaluating and not cls._contains_vars(expression)
        ):
            return expression

        context = cls.get_context(session, extra_context=extra_context)

        try:
            expr = env.compile_expression(expression.replace("{{", "").replace("}}", ""))
            return expr(context)
        except Exception:
            try:
    ...
    def render_request_conf(cls, session, data):
        if isinstance(data, str):
            ...
            list_data = []
            for i in data:
                list_data.append(cls.render_request_conf(session, i))
            ...
            for key, value in data.items():
                dict_data.update({key: cls.render_request_conf(session, value)})
            return dict_data
        else:
            return data
    ...
    def get_variables_from_response_obj(response):
        ...
        for i in inspect.getmembers(response):
            ...
            if not i[0].startswith("_"):
                # Ignores methods
                if not inspect.ismethod(i[1]) and type(i[1]) in (
                    str,
                    dict,
                    int,
                    dict,
                    ...
                    if len(i) == 2:
                        response_varialbes.update({i[0]: i[1]})
        ...
        if isinstance(response_content, dict):
            response_varialbes.update(response_content)

        # update content to save the one that has been decoded
        response_varialbes.update({"content": response_content})
    ...
    def fire_ussd_report_session_task(
        initial_screen: dict, session_id: str, support_countdown=True
    ):
        ussd_report_session = initial_screen["ussd_report_session"]
        args = (session_id,)
        kwargs = {"screen_content": initial_screen}
        keyword_args = ussd_report_session.get("async_parameters", {"countdown": 900}).copy()
        if not support_countdown and keyword_args.get("countdown"):
            del keyword_args["countdown"]

        report_session.apply_async(args=args, kwargs=kwargs, **keyword_args)
...
class UssdEngine(object):
    ...
    def ussd_dispatcher(self):
        ...
        self.ussd_request.session.update({"ussd_request": self.ussd_request.all_variables()})
        ...
        self.logger.debug("gateway_response", text=ussd_response.dumps(), input="{redacted}")
    ...
    def run_handlers(self):
        ...
        while not isinstance(ussd_response, UssdResponse):
            ...
            screen_type = (
                "initial_screen"
                if handler == "initial_screen" and isinstance(screen_content, str)
    ...
    def validate_ussd_journey(ussd_content: dict) -> (bool, dict):
        ...
        if "initial_screen" not in ussd_content.keys():
            is_valid = False
        ...
        for screen_name, screen_content in ussd_content.items():
            ...
            # Handle other screens as dictionaries
            if not isinstance(screen_content, dict):
                errors.update(dict(screen_name="Screen content should be a dictionary"))
                is_valid = False
                continue
            ...
            if base_errors:
                errors.update({screen_name: base_errors})
                is_valid = False
                continue
            ...
            screen_validation, screen_errors = handlers.validate(screen_name, ussd_content)
            if screen_errors:
                errors.update({screen_name: screen_errors})
    ...
    for key, value in error_response.items():
        if isinstance(value, list):
            if len(value) == 1 and key == "_schema":
                errors.append(dict(path=paths, message=value[0]))
            elif len(value) == 1 and value[0] == "This field is required.":
                errors.append(dict(path=paths, message=value[0].replace("This field", key)))
            else:
                errors.append(dict(path=paths + [key], message="\n".join(value)))
        else:
...
def render_journey_as_graph(ussd_screen: dict) -> Graph:
    ...
    _registered_ussd_handlers["initial_screen"](
        UssdRequest("dummy", "dummy", "", "en", journey_name="", journey_version=""),
        "initial_screen",
        initial_screen,
        initial_screen,
        raw_text=True,
...
def render_journey_as_mermaid_text(ussd_screen: dict) -> str:
    return convert_graph_to_mermaid_text(render_journey_as_graph(ussd_screen))
